...
//TODO убрть весь код, что задает стиль и сделать CSS файл
    ...
    private final String AUTHMSGWAIT = "Вы не авторизованы, сервере у вас есть 120с. Прошло";
    ...
    void makeMessage(String text, String id) {
        ...
        if(text.contains("Вы не авторизованы, сервере у вас есть 120с")){
            try {
            ...
            if(!fileLogName.isEmpty()) {
                writeHistoryChatLog( "Вход не выполнен");         //---------------------- ДЗ №3 для записи первой приветственной строки
            }
        }
...
//-------------------
      ...
      //  if (text.contains("Hello /client")) {
        if (text.contains("Hello /")) {
            ...
            if(!fileLogName.isEmpty()) {
                writeHistoryChatLog(text);
            }//здесь пишется основной лог, все, что отображается в чате проходит тут, уже без служебных комманд
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        Thread readT = new Thread(() -> {
            ...
            if (finalAuthMsg != null) {
                ...
                makeMessage("Рады видеть вас снова!Как обычно, можете видеть последние сообщения", RECEIVERID);
            }
    ...
    private String authInitialize(String title) throws IOException {
        ...
        String welcome = null;
        if(title.equals("anonymousUser")){
            nick = title;
            fileLogName = title + ".txt";
            out.writeUTF("anonymousUser");
           return in.readUTF();
        ...
        while (flag) {
            ...
            if (result.isPresent()) {
                if (title.equals("Регистрация")) {
                    fileLogName =  result.get().getValue() + ".txt";
                    returnMsg = "serverAuth" + "@/" + result.get().getValue() + "@/" + result.get().getKey();
                } else {
                    fileLogName = result.get().getKey() + ".txt";
                    returnMsg = "signIn" + "@/" + result.get().getValue() + "@/" + result.get().getKey();
                }
            }
        ...
        if (welcome.contains("Hello /")) {
            String[] splIttext = welcome.split("/");
            String[] subText = splIttext[1].split("&"); //
    ...
    private String firstEntry() throws IOException {
        ...
        return authInitialize("anonymousUser");
    }
//------------------------------------------------------------ДЗ №3
...
//функция для подгрузки истории сообщений. При вызове функции, передается ArrayList tmp в который мы считали данные из файла.
    ...
    public void logClicked(MouseEvent mouseEvent) throws IOException {
        makeMessage(authInitialize("Вход"),RECEIVERID);
    }
}